...
function createStars() {
  stars.length = 0;
  const count = reducedMotion ? 88 : mobile ? 190 : 330;
  for (let i = 0; i < count; i += 1) {
    stars.push({
      x: Math.random() * width,
      y: Math.random() * height,
      size: Math.random() < .08 ? random(1.45, 2.35) : random(.28, 1.28),
      alpha: random(.2, .88),
      twinkle: random(1.1, 3.7),
...
function createHeroMotes() {
  heroMotes.length = 0;
  const count = reducedMotion ? 168 : mobile ? 380 : 620;
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    heroMotes.push({
      ...
      wanderX: random(4, 24),
      wanderY: random(4, 20),
      size: Math.random() < .05 ? random(1.35, 2.1) : random(.22, 1.05),
      alpha: random(.46, 1),
...
function createEmbers() {
  embers.length = 0;
  const count = reducedMotion ? 20 : mobile ? 46 : 76;
  for (let i = 0; i < count; i += 1) {
    embers.push({
      phase: random(0, Math.PI * 2),
      radius: random(22, 118),
      speed: random(.24, .72),
      size: random(.75, 2.7),
      alpha: random(.2, .72)
    });
...
function drawStars(time) {
  for (const star of stars) {
    ...
    backCtx.shadowBlur = star.size > 1.35 ? star.size * 5 : 0;
    backCtx.arc(star.x, star.y, star.size * (.72 + pulse * .28), 0, Math.PI * 2);
    backCtx.fill();
    if (star.size > 1.4 && pulse > .82) {
      const ray = star.size * (2.8 + pulse * 1.7);
      backCtx.globalAlpha = alpha * .5;
      backCtx.lineWidth = .45;
      backCtx.strokeStyle = `rgba(${star.hue},.9)`;
      backCtx.beginPath();
      backCtx.moveTo(star.x - ray, star.y);
      backCtx.lineTo(star.x + ray, star.y);
      backCtx.moveTo(star.x, star.y - ray);
      backCtx.lineTo(star.x, star.y + ray);
      backCtx.stroke();
...
function drawHeroMotes(ctx, time, front) {
  ...
  for (const mote of heroMotes) {
    ...
    const shouldGlow = mote.glint || size > 1.15;
    ctx.shadowColor = shouldGlow ? `rgba(${color},.92)` : "transparent";
    ctx.shadowBlur = shouldGlow ? size * (front ? 8 : 5) : 0;
    ctx.beginPath();
...
function drawCandle(time) {
  ...
  const flameHeight = 54 * breath;
  const flameWidth = 15.2 + Math.sin(time * 6.4) * 1.35;
  const glowY = anchor.y - flameHeight * .46;

  const glow = backCtx.createRadialGradient(anchor.x + sway * .3, glowY, 0, anchor.x, glowY, 158 * breath);
  glow.addColorStop(0, "rgba(255,253,229,.94)");
  glow.addColorStop(.22, "rgba(255,193,91,.58)");
  glow.addColorStop(1, "rgba(255,164,77,0)");
  backCtx.fillStyle = glow;
  backCtx.fillRect(anchor.x - 178, glowY - 178, 356, 356);

  frontCtx.save();
  frontCtx.translate(anchor.x + sway, anchor.y);
  frontCtx.shadowColor = "rgba(255,177,62,.95)";
  frontCtx.shadowBlur = 34;
...
function startExperience() {
  ...
  burst(width * (mobile ? .5 : .72), height * .28, mobile ? 58 : 120, "pearl");
  startMusic();
  ...
  transitionTimer = setTimeout(() => {
    ...
    const point = heroPoint();
    burst(point.x, point.y, mobile ? 68 : 142, "pearl");
  }, reducedMotion ? 80 : 1650);
}

function acceptBouquet() {
  if (scene !== "bouquet") return;
  updateScene("lunar-transition");
  setPanel("bouquet-copy", false);
  setPanel("lunar-transition", true);
  const point = heroPoint();
  burst(point.x, point.y, mobile ? 130 : 280, "pearl");

  clearTimeout(transitionTimer);
  transitionTimer = setTimeout(() => {
    setPanel("lunar-transition", false);
    updateScene("cake");
    setPanel("birthday", true);
    const cakePoint = heroPoint();
    burst(cakePoint.x, cakePoint.y, mobile ? 110 : 230, "gold");
